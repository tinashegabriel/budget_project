...
class budget_expenses(models.Model):
    ...
    def action_approve_expense_sheets(self):
        request_bugdet_validation = self.env['budget.project'].search([('id','=',self.budget_id.id)],order="id desc",limit=1)
                ...
                if request_product_validation.qty > 0:
                    pass
                    
                else:

                    raise UserError(_('You do not have allocations in your bucket for this product '+line.requisition_line_ids.product_id.name))
